<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=q, initial-scale=1.0">

    <link rel="stylesheet" href="assets/css/style.css" type="text/css">
    <link rel="shortcut icon" href="assets/img/favicon.ico" type="image/x-icon">
    <title>Bhutan Developer Community</title>
</head>
<body>
    <div class="hero">
        <div class="row">
            <img src="assets/img/logo.png" alt="Logo" height="30%" width="30%"><br><br>
            <h1><font color="#fff">Coming Soon</font></h1><br>
            <h1>Bhutan Developer Community</h1>
         
            <hr>  
        </div>

        <div class="row">
            <a href="https://discord.gg/kfG4Z9qBEb"><img class="social-icon" src="assets/img/discord.png" alt="Discord" height="45"></a>
            <a href="https://chat.whatsapp.com/ByKjpnV2ajsBiqG140WEI2"><img class="social-icon" src="assets/img/whatsapp.png" alt="Whats App" height="45"></a>
            <a href="https://github.com/BTDeveloperCommunity"><img class="social-icon" src="assets/img/github.png" alt="Github" height="45"></a>
            <br>
            <p>Join our Community Today!</p>

        </div>
    </div>
 
</body>
</html>
